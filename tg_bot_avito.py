...
async def star_comand(message: types.Message):
    await message.reply("Привет! Я выгружаю с авито объявления по поиску производственных помещений в пределах Санкт-Петербурга и области. Цена - до 500 р. кв. м. Напиши сколько последних объявлений (от 2 до 50) выгрузить и увидишь результат.")
...
async def get_avito_ad(message: types.Message):
    ...
    try:
        ...
        url = "https://www.avito.ru/sankt-peterburg/kommercheskaya_nedvizhimost/sdam/proizvodstvo-ASgBAQICAUSwCNRWAUCeww0UjNk5?f=ASgBAQECAkSwCNRW9BKo2gEBQJ7DDRSM2TkBRcaaDBN7ImZyb20iOjAsInRvIjo1MDB9"
        ...
        try:
            ...
            for ad in all_ad_on_page[:numbers_of_ads]:
                ...
                count += 1
                await message.reply(f"Обработано объявлений: {count} из {numbers_of_ads}")
